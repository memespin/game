...
const WALLET_CONNECT_PROJECT_ID = 'b563e6e9697abbb05502b830f7d0c503';
const INFURA_ID = '65466506adfd40bb907fe06722bc70de';

export const CONTRACT_ABI = ABI;
export const CONTRACT_ADDRESS = window.ENV.CONTRACT_ADDRESS;
export const NFT_CONTRACT_ADDRESS = window.ENV.NFT_CONTRACT_ADDRESS;
export const CHAIN_ID = window.ENV.CHAIN_ID;
...
const chainConfigs = {
  '80002': {
    ...
    rpcUrls: [
      `https://polygon-amoy.infura.io/v3/${INFURA_ID}`,
...
export const mapChoice = (choiceNumber) => {
  switch (choiceNumber) {
    case 0:
      return CHOICES.ELON;
    case 1:
...
export const reverseMapChoice = (choiceString) => {
  switch (choiceString) {
    case CHOICES.ELON:
      return 0;
    case CHOICES.PEPE:
...
let rpcUrlIndex = 0;
let cachedRoundHistory = null;

const logContractCall = async (name, call) => {
  console.log(`🔄 Requesting ${name}...`);
  try {
    const result = await call();
    console.log(`✅ Received ${name}:`, result);
    return result;
  } catch (error) {
...
const tryRpcConnection = async (chainId) => {
  const urls = chainConfigs[chainId].rpcUrls;
  while (rpcUrlIndex < urls.length) {
    try {
      const provider = new ethers.providers.JsonRpcProvider(urls[rpcUrlIndex]);
      const blockNumber = await provider.getBlockNumber();
      console.log(`RPC connection successful to ${urls[rpcUrlIndex]}, block: ${blockNumber}`);
      return provider;
    } catch (error) {
      console.log(`RPC connection failed for ${urls[rpcUrlIndex]}:`, error.message);
      rpcUrlIndex++;
    }
  }
  rpcUrlIndex = 0; // Reset for next attempt
  throw new Error('All RPC connections failed');
};

const addOrSwitchChain = async (provider, targetChainId) => {
  ...
  try {
    ...
    if (switchError.code === 4902 || switchError.code === -32603) {
      console.log('Chain not added yet, attempting to add...');
      try {
        ...
        await provider.request({
          ...
          params: [{
            ...
            networkId: chainConfig.networkId?.toString(),
            shortName: chainConfig.shortName,
            chain: chainConfig.chain,
            networkType: chainConfig.networkType,
            explorers: chainConfig.explorers
          }]
        });
...
const initInjectedProvider = async () => {
  try {
    if (!window.ethereum) {
    ...
    // Try to get a working RPC first
    try {
      await tryRpcConnection(CHAIN_ID);
    } catch (error) {
      console.error('All RPC connections failed:', error);
      return false;
    }
    ...
    console.log('Current chain:', currentChainIdDecimal, 'Target chain:', CHAIN_ID);

    if (currentChainIdDecimal !== CHAIN_ID) {
      const switched = await addOrSwitchChain(window.ethereum, CHAIN_ID);
      if (!switched) {
        console.error('Failed to switch to correct chain');
        return false;
      }
      provider = new ethers.providers.Web3Provider(window.ethereum);
    }
    ...
    try {
      ...
      console.log('Connected to wallet:', currentWallet);
    } catch (connectError) {
      console.error('Connection rejected:', connectError);
      return false;
    }
    ...
    console.error('Injected provider error:', error);
    return false;
  }
};
...
const initWalletConnect = async () => {
  try {
    // Clear any existing WC sessions
    localStorage.clear();
    ...
    wcProvider = await EthereumProvider.init({
      ...
      chains: [parseInt(CHAIN_ID)],
      optionalChains: [],
      showQrModal: true,
      rpcMap: {
        [parseInt(CHAIN_ID)]: chainConfigs[CHAIN_ID].rpcUrls[rpcUrlIndex]
      },
      metadata: {
        name: 'Memespin',
        description: 'Memespin Game',
        url: window.location.origin,
        icons: ['https://memespin.io/images/MEMESPIN-H1.png']
      }
    });
    ...
    const connection = await wcProvider.connect();
    console.log('WalletConnect connection:', connection);
    ...
    if (connectedChainId !== CHAIN_ID) {
      const switched = await addOrSwitchChain(wcProvider, CHAIN_ID);
      if (!switched) {
        console.error('Failed to switch to correct chain');
        return false;
      }
...
export const initializeProvider = async () => {
  if (isInitialized && provider && (currentWallet || wcProvider)) {
    return true;
  }

  if (window.ethereum && await initInjectedProvider()) {
    isInitialized = true;
    return true;
  }
...
export const fetchRoundHistory = async (address, currentRoundId, forceRefresh = false) => {
  ...
  // Use cached history unless forced refresh
  if (cachedRoundHistory && !forceRefresh) {
    console.log('📋 Using cached history');
    return cachedRoundHistory;
  }

  let history = [];
  for (let i = Math.max(1, currentRoundId - 20); i < currentRoundId; i++) {
    const playerInfo = await logContractCall(`roundHistory_${i}`, 
      () => contract.getPlayerInfo(i, address)
    );
...
export const fetchGameState = async (address = null) => {
  ...
  try {
    const [contractInfo, currentRoundId, currentRoundInfo] = await Promise.all([
      logContractCall('contractInfo', () => contract.getContractInfo()),
      logContractCall('currentRoundId', () => contract.currentRoundId()),
      logContractCall('currentRoundInfo', async () => {
        const roundId = await contract.currentRoundId();
        return contract.getRoundInfo(roundId);
      })
    ]);
    ...
    if (address) {
      [currentPlayerInfo] = await Promise.all([
        logContractCall('playerInfo', () => contract.getPlayerInfo(currentRoundId, address))
      ]);

      if (currentRoundId > 1) {
        [previousRoundInfo, previousPlayerInfo] = await Promise.all([
          logContractCall('previousRoundInfo', () => contract.getRoundInfo(currentRoundId - 1)),
          logContractCall('previousPlayerInfo', () => contract.getPlayerInfo(currentRoundId - 1, address))
        ]);

        // Check if user won or drew in the previous round
        if (previousPlayerInfo && 
            (previousPlayerInfo.result === 1 || previousPlayerInfo.result === 2)) { // 1=Win, 2=Draw
          console.log('🎉 Win/Draw detected in previous round, refreshing history...');
          await fetchRoundHistory(address, currentRoundId, true);
        }
      }

      // Load history only if we don't have it cached
      if (!cachedRoundHistory) {
    ...
    const gameState = {
      ...
      roundHistory: cachedRoundHistory || [],
    ...
    if (cachedRoundHistory) {
      const totalPending = cachedRoundHistory.reduce((total, round) => {
    ...
    console.log('📊 Compiled game state:', gameState);
    return gameState;
  } catch (error) {
...
export const refreshRoundHistory = async (address, currentRoundId) => {
  return fetchRoundHistory(address, currentRoundId, true);
};

export const playGame = async (choice, referrer) => {
  if (!contract) await initializeProvider();
  const choiceIndex = reverseMapChoice(choice);
  const playCost = await logContractCall('playCost', () => contract.playCost());
  ...
  try {
    const tx = await logContractCall('playTransaction', 
      () => contract.play(choiceIndex, referrer, { value: playCost })
    );
    const receipt = await logContractCall('playReceipt', () => tx.wait());

    // Force refresh history after successful play
    if (currentWallet) {
      await fetchRoundHistory(currentWallet, await contract.currentRoundId(), true);
    }
...
export const endRound = async () => {
  if (!contract) await initializeProvider();
  try {
    const tx = await logContractCall('endRound', 
      () => contract.handleRoundTransition()
    );
    const receipt = await logContractCall('endRoundReceipt', () => tx.wait());
    return receipt;
  } catch (error) {
...
export const claimRewards = async () => {
  if (!contract || !cachedRoundHistory) await initializeProvider();
  try {
    const claimableRounds = [];
    
    // Use cached history to check for claimable rounds
    for (const round of cachedRoundHistory) {
      if (!round.hasClaimed && round.result !== 'Pending' && round.result !== 'Lose' && parseFloat(round.reward) > 0) {
        claimableRounds.push(round.id);
      }
    }
...
export const checkNFTOwnership = async (address) => {
  if (!nftContract) await initializeProvider();
  try {
    const balance = await logContractCall('nftBalance', 
...
export const mintNFT = async () => {
  if (!nftContract) await initializeProvider();
  try {
    const mintCost = await logContractCall('mintCost', () => nftContract.mintCost());
    const tx = await logContractCall('mintTransaction', 
      () => nftContract.mint({ value: mintCost })
    );
    const receipt = await logContractCall('mintReceipt', () => tx.wait());
    return receipt;
  } catch (error) {
...
export const getCurrentWalletAddress = () => currentWallet;
export const getProvider = () => provider;
export const getNativeCurrency = (chainId) => chainConfigs[chainId]?.nativeCurrency?.symbol || 'ETH';
